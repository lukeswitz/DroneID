...
def process_decoded_data(dc, pub):
    """Processes and forwards the decoded Bluetooth/Wi-Fi data and FPV detections."""

    # FPV Detection Processing (should not reach here due to earlier filtering, but kept for safety)
    ...
    # --- BLE path: Enhanced to support both AUX_ADV_IND and ADV_EXT_IND ---
    ...
    # --- Wi-Fi path: Publish as a list for consistency with BLE ---
    elif "DroneID" in dc:
        for mac, field in dc["DroneID"].items():
            if verbose:
                print("Open Drone ID WIFI\n-------------------------\n")

            merged = []  # Accumulate decoded messages to publish as a list
            ...
            if "AdvData" in field:
                try:
                    ...
                    for field_decoded in fields:
                        ...
                        merged.append(field_decoded)
                except Exception as e:
                    log("Decoding Error:", e)
            else:
                # No AdvData present — still emit a list with a minimal entry
                entry = {"MAC": mac}
                if "AUX_ADV_IND" in dc:
            ...
            # Publish once as a JSON array (BLE-style)
            if merged:
...
def main():
    ...
    for client in clients:
        ...
        try:
            ...
            # Subscribe to ALL messages to support BT/Wi-Fi/FPV Detection formats
